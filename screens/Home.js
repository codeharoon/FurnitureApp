...
const Home = ({navigation}) => {
  ...
  const filterCategory = (category) => {
    ...
    const filteritem = restaurantData.filter((item) => {
      return item.categories.includes(category.id);
    });
    setRestaurants(filteritem);
  };

  const categoryOfItem = (item) => {
    const cateitem = categories.filter((arr) => {
      return arr.id === item;
    });
    if (cateitem.length > 0) {
      return cateitem[0].name;
    } else {
      return '';
    }
  };
  ...
  const renderRestuarants = () => {
      ...
      return (
        <TouchableOpacity
          ...
          <View style={{flexDirection: 'row', marginTop: SIZES.padding}}>
            ...
            {item.categories.map((cate, index) => {
              return (
                <View
                  ...
                  <Text style={{...FONTS.body3}}>{categoryOfItem(cate)}</Text>
            ...
            {[1, 2, 3].map((itemrating) => (
              <Text
                key={`dollarsign${itemrating}`}
                style={{
                  ...FONTS.body3,
                  
                  color:
                    itemrating <= item.priceRating
                      ? COLORS.black
                      : COLORS.darkgray,
                }}>
                $
    ...
    return (
      <FlatList
        style={{paddingHorizontal: SIZES.padding, paddingBottom: 30}}
        data={restaurants}
        showsVerticalScrollIndicator={false}
        keyExtractor={(item) => item.id.toString()}
        renderItem={({item, index}) => renderres(item, index)}></FlatList>
    );
  };
